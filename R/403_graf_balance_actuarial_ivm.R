...
load( paste0( parametros$RData_seg, 'IESS_IVM_balances_', "escenario_1", '.RData' ) )
aux_1<- as_tibble(balance_anual[ , t := t + parametros$anio_ini ]) %>%
  select(t,A_1:=A2_vap,B_1:=B_vap,V_1:=V)

load( paste0( parametros$RData_seg, 'IESS_IVM_balances_', "escenario_2", '.RData' ) )
...
ggsave( plot = compa_bal_act, 
        filename = paste0( parametros$resultado_graficos, 'iess_compa_bal_act_ivm', parametros$graf_ext ),
        width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
...
ggsave( plot = compa_ing_act, 
        filename = paste0( parametros$resultado_graficos, 'iess_compa_ing_act_ivm', parametros$graf_ext ),
        width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
