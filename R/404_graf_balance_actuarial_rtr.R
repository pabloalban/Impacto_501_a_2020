...
load( paste0( parametros$RData_seg, 'IESS_RTR_balances_', "escenario_1", '.RData' ) )
aux_1<- as_tibble(balance_anual[ , t := t + parametros$anio_ini ]) %>%
        select(t,A_1:=A2_vap,B_1:=B_vap,V_1:=V)

load( paste0( parametros$RData_seg, 'IESS_RTR_balances_', "escenario_2", '.RData' ) )
...
y_lim <- c( min( aux$V_1 )*0.9, max( aux$V_1 ) )
y_brk <- unique( pretty( seq(y_lim[1], y_lim[2], length.out = 5) ) )
y_lim <- c( min(y_brk), max(y_brk) ) # redefiniendo limites por razones estéticas
y_lbl <- formatC(y_brk/1e6, digits = 0, format = 'f', big.mark = '.', decimal.mark = ',')
...
ggsave( plot = compa_bal_act, 
        filename = paste0( parametros$resultado_graficos, 'iess_compa_bal_act_rtr', parametros$graf_ext ),
        width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
...
compa_ing_act <-  ggplot() +
                ...
                #  scale_linetype_manual("", values=c(1,20)) +
                  theme_bw() +
                  plt_theme +
                  theme(legend.position="bottom") +
                  theme( axis.text.x = element_text(angle = 90, hjust = 1 ) )
...
ggsave( plot = compa_ing_act, 
        filename = paste0( parametros$resultado_graficos, 'iess_compa_ing_act_rtr', parametros$graf_ext ),
        width = graf_width, height = graf_height, units = graf_units, dpi = graf_dpi )
